...
rm -rf ${LIBS_DIR}
mkdir -p ${LIBS_DIR}/{ios,macosx}/include

curl -Lk https://github.com/google/boringssl/archive/chromium-stable.zip | tar -xzp --strip-components=1 -C ${BORINGSSL_SRC_DIR}

cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_OSX_SYSROOT=macosx          -DBORINGSSL_PREFIX=OBJECTIVEPGP -DCMAKE_OSX_DEPLOYMENT_TARGET="10.11" -DCMAKE_OSX_ARCHITECTURES=x86_64 -H"${BORINGSSL_SRC_DIR}" -B"${BORINGSSL_BUILD_DIR}/macosx"
